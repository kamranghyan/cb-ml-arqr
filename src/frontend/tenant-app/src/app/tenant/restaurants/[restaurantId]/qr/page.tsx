...
export default function BranchQrPage() {
  ...
  // Tables are created/deleted in Settings → Tables. Here we only refresh.
      ...
      <div style={{ flex: 1, padding: '24px 32px', overflowY: 'auto', background: C.bg, display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        {loading && (
          <div style={{ padding: 60, textAlign: 'center', color: C.muted }}>
            <Loader2 size={22} style={{ animation: 'spin 1s linear infinite' }} /> Loading tables…
          </div>
        )}
        {!loading && loadError && (
        ...
        {!loading && !loadError && records.length === 0 && (
          <div style={{ padding: 60, textAlign: 'center', color: C.subtle }}>
            ...
            <p style={{ margin: '4px 0 0', fontSize: 13 }}>
              Create tables in <strong>Settings → Tables</strong>, then refresh here to generate QR codes.
            </p>
          </div>
        )}
