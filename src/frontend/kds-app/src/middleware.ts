...
export function middleware(request: NextRequest) {
  ...
  const idToken = request.cookies.get('menulay_token_kds')?.value
  ...
  if (pathname.startsWith('/kds')) {
    ...
    const groups = (claims['cognito:groups'] as string[]) ?? []
    // Only kitchen staff belong on this screen. A tenant owner who needs to
    // watch orders can do so from the tenant console.
    if (!groups.includes('menulay_kitchen_staff')) {
