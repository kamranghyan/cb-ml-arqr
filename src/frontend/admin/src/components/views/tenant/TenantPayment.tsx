...
import { getValidToken } from '@/lib/cognito';
...
export default function TenantPayment() {
    ...
    const checkPaymentStatus = async (
        ...
        for (
            ...
            try {
                /*
                 * IMPORTANT:
                 * Get a fresh ACCESS token for every
                 * status request.
                 *
                 * Do not reuse the token from
                 * payment initiation.
                 */
                const token =
                    await getValidToken();
                ...
                const statusUrl =
                    `/api/payment-svc/payment/status/${encodeURIComponent(
                        orderId
                    )}`;

                console.log(
                    `[PAYMENT] Status check ${attempt + 1}/${maxAttempts}`,
                    {
                        orderId,
                        url: statusUrl,
                    }
                ...
                if (!statusRes.ok) {
                     ...
                     *
                     * then the request is being rejected
                     * by API Gateway BEFORE FastAPI.
                     */
                    throw new Error(
                        statusData?.message ||
                        statusData?.detail ||
                        statusData?.error ||
                        `Payment status request failed (${statusRes.status})`
                    );
                }
        ...
        throw new Error(
            'Payment confirmation is taking longer than expected. Please check your subscription shortly.'
        );
    };
    ...
    // Handle payment
    // ─────────────────────────────────────────────
    const debugCognitoToken = (token: string) => {
        try {
            ...
            console.log('[PAYMENT] 🔐 COGNITO TOKEN DEBUG', {
                issuer: payload.iss,
            ...
            console.error(
                '[PAYMENT] ❌ Cannot decode Cognito token:',
                error
            );
        }
    };
