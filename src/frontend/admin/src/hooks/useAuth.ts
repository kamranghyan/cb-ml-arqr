...
export function useAuth() {
  ...
  const login = useCallback(async (
    ...
    try {
      ...
      setCookie('menulay_token_console', result.tokens!.idToken, 30)

      setState({ user, loading: false, error: null })

      // Determine redirect based on group
      // This app only serves tenant owners; the middleware sends anyone
      // else to their own console.
      // One login serves both consoles; the role decides where to land.
      // One dashboard for every console role — the page renders the view
      // that matches whoever signed in.
  ...
  const logout = useCallback(async () => {
    ...
    deleteCookie('menulay_token_console')
    setState({ user: null, loading: false, error: null })
