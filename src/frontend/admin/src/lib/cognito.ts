...
/**
 * The ID token as it stands, without a refresh round trip.
 *
 * Used where a render needs to know who is signed in — the guards and the
...
export function extractUser(idToken: string): AuthUser {
  ...
  // Prefer the group-derived role; custom:role is legacy and often unset.
