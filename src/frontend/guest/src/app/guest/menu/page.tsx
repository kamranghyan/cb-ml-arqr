      ...
      <div style={{ padding: '52px 20px 0', background: D.bg, position: 'sticky', top: 0, zIndex: 50 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
          ...
          <button onClick={() => router.push('/guest/cart')} style={{ width: 40, height: 40, borderRadius: 12, background: BRAND, border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', position: 'relative' }}>
            ...
            {cartCount > 0 && <span style={{ position: 'absolute', top: -5, right: -5, width: 18, height: 18, borderRadius: '50%', background: '#fff', color: BRAND, fontSize: 10, fontWeight: 800, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{cartCount}</span>}
          </button>
        </div>
          ...
          {filtered.map(item => (
            <div key={item.id} onClick={() => router.push(`/guest/menu/${item.id}`)}
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{ fontFamily: "'Baloo 2', sans-serif", fontSize: 16, fontWeight: 600, color: D.text, margin: '0 0 3px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.name}</p>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <span style={{ fontFamily: "'Baloo 2', sans-serif", fontSize: 16, fontWeight: 700, color: BRAND }}>Rs. {item.price.toLocaleString()}</span>
                  <button onClick={e => handleAdd(item, e)}
                    style={{ width: 32, height: 32, borderRadius: 10, background: added[item.id] ? '#22c55e' : BRAND, border: 'none', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', transition: 'all 0.2s', boxShadow: added[item.id] ? 'none' : '0 3px 10px rgba(255,87,35,0.3)' }}>
...
export default function MenuPage() {
  return (
    <Suspense fallback={<div style={{ minHeight: '100dvh', background: '#111', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Loader2 size={28} color="#ff5723" className="animate-spin" /></div>}>
      <MenuContent />
    </Suspense>
  );
}