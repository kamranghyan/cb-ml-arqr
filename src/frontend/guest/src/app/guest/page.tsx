...
// The restaurant comes from the QR code — there is no default branch.
const API_BASE  = process.env.NEXT_PUBLIC_API_BASE  || 'https://g1ou0w5x4m.execute-api.ap-south-1.amazonaws.com/dev';
// The tenant is resolved server-side from the restaurant; the guest never sees it.
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '0 20px' }}>
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          {loading ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              {popular.map(item => (
                <Link key={item.id} href={`/guest/menu/${item.id}?rid=${qrRid}&tid=${tid}`} style={{ textDecoration: 'none' }}>
