...
from app.models.base import ValidationError as ModelValidationError
...
def register_exception_handlers(app: FastAPI) -> None:
    ...
    async def forbidden_handler(r: Request, exc: ForbiddenError):
        return JSONResponse(
            status_code=403, content=_err(exc.error_code, exc.message)
        )

    @app.exception_handler(ResourceNotFoundError)
    async def not_found_handler(r: Request, exc: ResourceNotFoundError):
        return JSONResponse(
            status_code=404, content=_err(exc.error_code, exc.message)
        )
    ...
    async def validation_handler(r: Request, exc: ValidationError):
        return JSONResponse(
            status_code=422, content=_err(exc.error_code, exc.message)
        )

    @app.exception_handler(ModelValidationError)
    async def model_validation_handler(r: Request, exc: ModelValidationError):
        return JSONResponse(
            status_code=400,
            content=_err("VALIDATION_ERROR", str(exc.errors)),
        )
